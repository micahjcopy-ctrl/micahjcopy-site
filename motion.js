...
// PERF: throttled to 300ms and self-removing once every section is in —
// unthrottled getBoundingClientRect on every scroll event caused layout
// thrash / dropped frames under Lenis (scroll-lag QA 2026-07-10).
var failsafeLast = 0;
var failsafe = function () {
var now = Date.now();
if (now - failsafeLast < 300) return;
failsafeLast = now;
var pending = 0;
sections.forEach(function (s) {
...
if (want === scrolledState) return; // PERF: skip redundant class writes per frame
scrolledState = want;
if (want) header.classList.add("scrolled");
