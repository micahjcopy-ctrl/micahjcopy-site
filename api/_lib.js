...
const FLOWS = [
  { key: "cart", name: "Cart abandon flow", share: 0.30, effort: 1, when: "day 1–7", steps: [
    ...
    "Check channel eligibility + quiet hours — most accounts only reach 10–20% of abandoners, and that setting is why.",
...
function computeReport(i) {
  ...
  if (i.blastPct >= 50) fixes.push({
    name: "Stop blasting the full list", dollars: campLeak, effort: 1, when: "day 1–7", steps: [
      ...
      "Expect open rates to jump within 2 weeks — deliverability repairing itself protects every flow you fix after this." ]});
  if (fixes.length === 0) fixes.push({
    name: "Optimize send-through + RPR on live flows", dollars: emailRev * 0.05, effort: 2, when: "day 1–14", steps: [
      "Audit entry triggers + channel eligibility per flow — most 'complete' accounts still only reach a fraction of eligible volume.",
...
function sequenceEmail(day, inputs, reportId) {
  ...
  const emails = {
    1: { subject: `Your first ${fmt(f1.dollars)}/mo is the easy one`,
      text: `Micah here. Your report says "${f1.name}" is worth about ${fmt(f1.dollars)}/mo — and it's ranked #1 because it's the highest dollars-per-effort on your list, not because it's fancy.
...
ONE: re-run your Retention Score (free — same place you got the first one: ${SITE_URL}/score). Whatever it says becomes your new baseline — and in 60 days you can re-run it again and put a number on this week.
